...
export async function POST(req: Request) {
  ...
  const webhookEvent = await prisma.webhookEvent.create({
    data: { provider: "whatsapp", payload: payload as Prisma.InputJsonValue, status: "RECEIVED" },
  });
  ...
  try {
    ...
    for (const entry of payload.entry ?? []) {
      ...
      const integration = await prisma.integration.findUnique({
        where: { routingKey: `WHATSAPP:${entry.id}` },
      });
      if (!integration || integration.status !== "CONNECTED") continue;
      businessIds.add(integration.businessId);

      for (const msg of whatsAppProvider.parseWebhookPayload({ entry: [entry] })) {
